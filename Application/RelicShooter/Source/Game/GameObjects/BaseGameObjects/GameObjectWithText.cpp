...
{
	px = static_cast<int>(this->m_Text.getPosition().x);
	py = static_cast<int>(this->m_Text.getPosition().y);
}
...
void GameObjectWithText::Initialise()
{
	...
	m_Text.Initialise(this->m_Font.c_str(),this->m_String.c_str(),this->m_Colour,this->m_Size, m_isUIObject);
}
...
void GameObjectWithText::SetCharSpacing(float spacing) {
